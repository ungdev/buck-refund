  ...
  return (
    <AppModal>
      {state === PageState.LOGIN && (
        <>
          <h2 className={styles.title}>{t('common:login.magic.success')}</h2>
          <p>{t('common:login.magic.success.2')}</p>
        </>
      )}
      {state === PageState.SENT && (
        <>
          <h2 className={styles.title}>{t('common:login.magic.sent')}</h2>
          <p>{t('common:login.magic.sent.2')}</p>
        </>
      )}
      {state === PageState.ERROR && (
        <>
          <h2 className={styles.title}>{t('common:login.magic.error')}</h2>
          <p>{t('common:login.magic.error.2')}</p>
        </>
      )}
      {state === PageState.LOADING && (
        <>
          <h2 className={styles.title}>{t('common:login.loading')}</h2>
      ...
      {state === PageState.READY && (
        <>
          <h2 className={styles.title}>{t('common:login.magic.generate')}</h2>
          <Input
            value={email}
